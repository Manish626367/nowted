  ...
  return getNote.deletedAt ? (
    ...
    <div className="p-10 flex flex-col gap-6 relative">
      ...
      <div>
        ...
        <div className="flex gap-10 font-semibold py-6 cursor-pointer "  > 
          <div className="flex gap-4 text-gray-300 cursor-pointer">
            <img src={closeFolderIcon} alt="Folder Icon" />
            <p>Folder</p>
          </div>
          <p onClick={() => setShowDropdown(!showDropdown)} className="border-b border-gray-300">{getNote.folder.name}</p>

          {showDropdown && (
            <div className="absolute max-h-70 overflow-y-auto scrollbar-hide top-60 left-28 bg-gray-800 text-white border border-gray-600 rounded-lg shadow-md p-2 w-48 z-10">
              {folders.map((folder) => (
                <div
                  key={folder.id}
